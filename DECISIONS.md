...
**Why:** Applying filters destructively at save time removes the live filter chain. It would also require rendering the processed result with `OfflineAudioContext` and then encoding that rendered output into a file format, adding complexity and introducing a processing step before uploads. Storing raw audio keeps saves fast (just write the blob to disk) and preserves the ability to revisit and adjust filter settings after saving.
...
Adding a new filter becomes a single registry entry. The UI controls, default state, basic server-side validation, and audio node creation all derive from the same definition, so they can't drift out of sync.
...
**Alternatives:** Using Web Audio API for both capture and processing gives maximum control but requires buffer capture via `AudioWorklet`, manual WAV/WebM encoding, and careful resource management — substantial complexity for a feature (live monitoring) that wasn't in scope.
...
**Alternatives:** Drag-and-drop reordering would give users more creative control, but adds significant UI complexity (drag state, reorder animations, order persistence) and requires users to understand signal flow to get good results.

## 5. Filesystem + JSON Persistence

**What:** Audio files are stored in `public/uploads/` and metadata in `data/clips.json`.

**Why:** Zero setup for reviewers — `npm install && npm run dev` is all that's needed. The JSON file is human-inspectable (easy to verify data integrity during review), and the data model is simple enough that a flat file handles it well. The tradeoff is accepting the lack of transactional guarantees and concurrent write safety, which is acceptable for a single-user demo.
...
**Alternatives:** Progressive enhancement with feature detection. Worth doing for a production app, but the effort-to-value ratio is low for a take-home where the reviewer will use Chrome.
...
## 8. Shared MediaElement Playback
...
## 9. Waveform Shows Raw Audio, Not Filtered
...
**Why:** Showing a filtered waveform would require rendering the audio through an `OfflineAudioContext` with the full filter chain every time filter parameters change. That's expensive, introduces visible latency on parameter adjustments, and adds complexity for a visual that users don't directly control. The waveform's purpose is to show the recording's amplitude shape and current playback position, while the native `<audio>` element remains the transport control.
...
## 10. Delay Tail Handling via Lifecycle Callbacks
...
**Why:** This keeps lifecycle logic co-located with the filter definition rather than leaking delay internals into the player. The player calls the callbacks without needing filter-specific logic.
...
## 11. Live Parameter Updates During Playback
...
The graph is only rebuilt when its structure changes (i.e., when filters are enabled or disabled). Plain parameter updates are handled separately through an `update` callback on each filter result. That callback lives next to `createNode` in the registry, so each filter owns both how it is created and how its live parameters are updated. The player does not need filter-specific logic; it just calls `result.update(params)`.
