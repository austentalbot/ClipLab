...
**Why:** Rebuilding the entire audio graph on every parameter change causes an audible gap — the old chain is torn down, a new chain is constructed, and there's a brief silence in between. Since Web Audio `AudioParam` values can be updated in-place without interrupting the signal flow, parameter changes are handled by a separate effect that calls an `update` callback on each filter's node result. The graph-rebuild effect depends on a stable `graphKey` string (the comma-joined list of enabled filter types), so it only fires when the enabled set actually changes.

Each filter's `update` function is defined in the registry alongside `createNode`, keeping the update logic co-located with the node creation logic. The player doesn't need to know which `AudioParam` each filter exposes — it just calls `result.update(params)`.

**Alternatives:** Using `AudioParam.linearRampToValueAtTime()` or `setTargetAtTime()` would provide smoother transitions for rapid slider movements, but adds complexity and is unnecessary for the current UI where discrete slider steps are small enough that direct `.value` assignment sounds smooth.
